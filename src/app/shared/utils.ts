...
export function canConvertTo<T>(input: any): boolean {
  try {
    const result = <T>input || (input as T);
...
export function convertTo<T>(input: any): T | null {
  if (canConvertTo<T>(input)) {
    return <T>input;
  }
  return null;
}

export function convertObject<T>(input: any): ConvertResult<T> {
  let result: ConvertResult<T> = {
    error: null,
    isFailure: false,
    item: <T>{}
  };
  ...
  if (!isObject(input)) {
    ...
    result.error = "Invalid type, input isn't an aboject.";
  }
